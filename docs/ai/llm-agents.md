...
### ⭕ Temperature와 샘플링 파라미터 ◑

| 파라미터 | 설명 | 값 |
|----------|------|-----|
| Temperature | 출력의 무작위성 조절 | 0: 결정적, 1+: 창의적 |
| Top-p (Nucleus) | 누적 확률 기반 샘플링 | 0.9: 상위 90% 토큰에서 선택 |
| Top-k | 상위 k개 토큰에서 선택 | 40: 상위 40개 중 선택 |
| Max tokens | 최대 출력 토큰 수 | - |

**사용 지침:**
- 코드 생성, 사실 기반 응답: Temperature 0~0.3
- 창작, 브레인스토밍: Temperature 0.7~1.0
- Temperature와 Top-p 중 하나만 조정 권장

### ⭕ RLHF (Reinforcement Learning from Human Feedback) ◑

인간 피드백으로 모델을 정렬(alignment)하는 학습 기법이다.
...
보상 모델 없이 직접 선호도 데이터로 학습하는 간소화된 방법이다.
...
텍스트를 고차원 벡터로 변환하여 의미적 유사성을 계산한다.

| 유사도 측정 | 설명 | 범위 |
|-------------|------|------|
| 코사인 유사도 | 벡터 간 각도 | -1 ~ 1 |
| 유클리드 거리 | 벡터 간 직선 거리 | 0 ~ ∞ |
...
문서를 적절한 크기로 분할하는 기법이다.

| 전략 | 설명 |
|------|------|
| 고정 크기 | 일정 토큰/문자 단위 분할 |
| 문장/단락 기반 | 자연스러운 경계로 분할 |
| 재귀적 분할 | 계층적으로 분할 |
| 의미 기반 | 임베딩 유사도로 분할 |

**청크 설정 요소:**
- 청크 크기: 보통 500~1000 토큰
- 오버랩: 문맥 유지를 위해 10~20% 중복
- 메타데이터: 출처, 페이지 번호 등 포함
