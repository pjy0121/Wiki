...
| 알고리즘 | 점화식 | 분석 | 결과 |
|----------|--------|------|------|
| 병합 정렬 | T(n) = 2T(n/2) + n | a=2, b=2, log_2(2)=1, f(n)=n=n^1 → Case 2 | O(n log n) |
| 이진 탐색 | T(n) = T(n/2) + 1 | a=1, b=2, log_2(1)=0, f(n)=1=n^0 → Case 2 | O(log n) |
| 행렬 곱 (단순) | T(n) = 8T(n/2) + n² | a=8, b=2, log_2(8)=3, f(n)=n² → Case 1 | O(n³) |
...
동일한 키를 가진 요소들의 **상대적 순서가 정렬 후에도 유지**되는 정렬이다.

```
예시: (이름, 나이) 데이터를 나이순으로 정렬

원본: [(Kim, 25), (Lee, 30), (Park, 25), (Choi, 30)]

안정 정렬 (나이순):
[(Kim, 25), (Park, 25), (Lee, 30), (Choi, 30)]
  ↑ Kim이 Park 앞에 (원래 순서 유지)

불안정 정렬 (나이순):
[(Park, 25), (Kim, 25), (Choi, 30), (Lee, 30)]
  ↑ 순서 바뀔 수 있음
```

**안정 정렬이 중요한 경우:**
- 다단계 정렬 (먼저 이름순, 그다음 나이순)
- 데이터베이스 쿼리 결과의 일관성
