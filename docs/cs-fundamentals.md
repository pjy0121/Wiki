...
자동으로 사용하지 않는 메모리를 회수하는 메커니즘이다.

| 방식 | 설명 | 언어 |
|------|------|------|
| **수동 관리** | 개발자가 직접 할당/해제 | C, C++ |
| **참조 카운팅** | 참조 수가 0이면 해제 | Objective-C (ARC), Python (보조), Rust (일부) |
| **추적 GC** | 루트에서 도달 불가능한 객체 해제 | Java, Go, JavaScript |

### 가비지 컬렉션 알고리즘 ◑

| 알고리즘 | 설명 | 장점 | 단점 |
|----------|------|------|------|
| **Mark-and-Sweep** | 도달 가능 객체 표시 후 나머지 해제 | 순환 참조 처리 가능 | Stop-the-world |
| **Copying (Semi-space)** | 살아있는 객체만 새 영역으로 복사 | 단편화 없음 | 메모리 2배 필요 |
| **Mark-Compact** | Mark 후 살아있는 객체를 압축 | 단편화 해소 | 압축 오버헤드 |
| **Generational** | 세대별로 나누어 관리 | 대부분 Young에서 회수 | 복잡함 |
...
| 세대 | 설명 | GC 종류 |
|------|------|---------|
| **Young** | 새로 생성된 객체, 짧은 수명 | Minor GC (빈번, 빠름) |
| **Old** | 오래 살아남은 객체 | Major GC / Full GC (드물지만 느림) |
...
A와 B가 서로 참조 → rc가 절대 0이 안 됨 → 메모리 누수
```

**해결:** Python은 순환 참조 탐지기를 추가로 사용
...
```
입력: "<div>hello</div><div>world</div>"

Greedy (기본): <.*>  → "<div>hello</div><div>world</div>"
...
- [ ] 컴파일러 vs 인터프리터
- [ ] JIT 컴파일의 동작 원리
- [ ] 컴파일러 단계 (어휘 분석, 구문 분석, 의미 분석)
- [ ] UTF-8 vs UTF-16
- [ ] Big Endian vs Little Endian
- [ ] 동시성 vs 병렬성
- [ ] 가비지 컬렉션 알고리즘 (Mark-and-Sweep, Generational)
- [ ] 참조 카운팅과 순환 참조 문제
- [ ] 정규 표현식 기초 문법
